...
/*   Updated: 2021/02/18 17:32:37 by sabra            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		dd_float_check(char *line)
{
	if (ft_charcnt(line, '-') > 1 ||
			(ft_charcnt(line, '-') > 0 && line[0] != '-'))
		return (0);
	if (ft_charcnt(line, '.') > 1 ||
...
int		find_err(char *line, int id)
{
	...
	if (id == FLOAT)
		return (dd_float_check(line));
	if (id == STRING)
	{
		if (ft_strncmp(line, "R", ft_strlen(line)) != 0
			&& ft_strncmp(line, "A", ft_strlen(line)) != 0
